...
local function recieveKey(newValue, prevValue, address, item)
  if not config.ramconfig.single_world then
  ...
  [0xF36D] = {flag="ammo", type="delta", 
    receiveFunc=function(newValue, prevValue, address, item, their_user)
      ...
      if (delta < -0xFF) then
        -- death message
        newValue = 0
        printOutput(their_user .. " killed you.")
        deathQueue[their_user] = true
      end
...
-- Events that should not be synced
function eventException(i)
  -- Dam flood
  if (i == 59 or i == 123) then
    return true
  end
...
function getGUImessage(address, prevVal, newVal, user)
  ...
  if name and prevVal ~= newVal then
    ...
    elseif ramItems[address].type == "bit" then
      for b=0,7 do
        ...
        if (newBit ~= prevBit) then
          if (type(name) == 'string') then
            printOutput(user .. ": " .. name .. " flag " .. b .. (newBit and '' or ' Removed'))
          elseif (name[b]) then
            printOutput(user .. ": " .. name[b] .. (newBit and '' or ' Removed'))
          end
        end
      end
    ...
    elseif ramItems[address].type == "delta" then
      ...
      if (delta > 0) then
        if (type(name) == 'string') then
          printOutput(user .. ": " .. name .. (delta > 0 and " +" or " ") .. delta)
        elseif (name[newVal]) then
          printOutput(user .. ": " .. name[newVal])
        end
      end
...
function getRAM() 
  ...
  for address, item in pairs(ramItems) do
    ...
    if config.ramconfig.single_world then
      if generickeys and item.flag == "skey" then
        skip = true
      elseif not generickeys and item.flag == "gkey" then
        skip = true
      end
    else
